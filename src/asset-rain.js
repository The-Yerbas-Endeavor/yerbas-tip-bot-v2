...
async function validateAssetPayout(interaction, ctx) {
  ...
  const assetName = clean(interaction.options.getString('asset', true), 64);
...
async function distributeAsset({ ledger, creator, participants, assetName, amountUnits, reference, paymentType }) {
  ...
  try {
    ...
    if (!senderRows[0] || toUnits(String(senderRows[0].balance)) < amountUnits) {
    ...
    await connection.execute(
      ...
      [fromUnits(amountUnits), creator.id, assetName]
    );
    ...
    for (let index = 0; index < unique.length; index += 1) {
      ...
      const share = fromUnits(shareUnits);
...
async function runImmediate(interaction, ctx, participants, payout, label, paymentType) {
  ...
  await interaction.reply({
    content: [
      `🌧️ **${label}!**`,
      `${interaction.user} shared **${fromUnits(result.paidUnits)} ${payout.assetName}** among **${result.participantCount} users**.`,
      `Each received approximately **${fromUnits(result.shareUnits)} ${payout.assetName}**.`,
...
async function startDrop(interaction, ctx, payout, mode) {
  ...
  if (mode === 'trivia' && (!phrase || !answer)) throw new Error('Trivia mode requires both phrase and answer');
  ...
  const message = await interaction.reply({
    content: [
      `🌧️ **${payout.assetName} ${mode.toUpperCase()} DROP**`,
      `${interaction.user} is dropping **${fromUnits(payout.amountUnits)} ${payout.assetName}**.`,
      clean(interaction.options.getString('message')) ? `> ${clean(interaction.options.getString('message'))}` : null,
      instruction,
  ...
  collector.once('end', async () => {
    try {
      ...
      const result = await distributeAsset({
        ...
        ...payout,
        reference: `asset-rain-${mode}:${interaction.id}`,
        paymentType: `assetRain${mode[0].toUpperCase()}${mode.slice(1)}`
      });
      if (!result.participantCount) {
        await message.reply('Asset drop ended with no eligible winners. Nothing was deducted.');
        return;
      }
...
function addCommonOptions(subcommand) {
  return subcommand
    .addStringOption((o) => o.setName('asset').setDescription('Yerbas Asset name').setRequired(true).setMaxLength(64))
    .addStringOption((o) => o.setName('amount').setDescription('Total asset amount to distribute').setRequired(true))
    .addStringOption((o) => o.setName('message').setDescription('Optional rain message').setMaxLength(200));
}

export function buildAssetRainCommand(ctx) {
  return {
    data: new SlashCommandBuilder()
      .setName('asset-rain')
      .setDescription('Rain Yerbas Assets using multiple distribution modes.')
      ...
      .addSubcommand((s) => addCommonOptions(s.setName('drop').setDescription('Start an interactive asset drop.'))
    ...
    async execute(interaction) {
      ...
      const payout = await validateAssetPayout(interaction, ctx);
      if (subcommand === 'all') {
        await runImmediate(interaction, ctx, await databaseUsers(ctx.ledger, interaction.user.id), payout, 'Asset Rain All', 'assetRainAll');
        return;
      }
      if (subcommand === 'online') {
        await interaction.guild.members.fetch({ withPresences: true });
        await runImmediate(interaction, ctx, onlineGuildUsers(interaction), payout, 'Online Asset Rain', 'assetRainOnline');
        return;
      }
      if (subcommand === 'active') {
        ...
        await runImmediate(interaction, ctx, users, payout, 'Active Asset Rain', 'assetRainActive');
        return;
      }
